...
df = spark.sql("SELECT   AAPL_adjClose, \
     ...
     WHERE DATE_FORMAT(AAPL_dateTime,'HHmm') =='1600' and DATE_FORMAT(AAPL_dateTime,'yyyy-MM-dd') between '2005-01-03' and '2021-12-31'\
...
df = spark.sql("SELECT   AAPL_adjClose, \
     ...
     WHERE DATE_FORMAT(AAPL_dateTime,'HHmm') =='1600' and DATE_FORMAT(AAPL_dateTime,'yyyy-MM-dd') between '2021-01-01' and '2022-12-31'\
     ORDER BY AAPL_dateTime asc \
     ")
     """
...
df_pd_pct = df_pd.pct_change()

# Option 1: drop rows with NaN values
#df_pd_pct_dropna = df_pd_pct.dropna().T

# Option 2: use imputer with NaN values
df_pd_pct_imp = pd.DataFrame(SimpleImputer().fit_transform(df_pd_pct), columns = df_pd_pct.columns)

df_pd_pct_dropna = df_pd_pct_imp.T

# define normalizer
normalizer = Normalizer()
new = normalizer.fit_transform(df_pd_pct_dropna)
...
df_result = pd.DataFrame({'labels': labels, 'companies': df_pd_pct.columns})

# Display df sorted by cluster labels
display(df_result.sort_values('labels'))
