...
data = spark.sql("SELECT AAPL_adjClose, \
     ...
     WHERE DATE_FORMAT(AAPL_dateTime,'HHmm') between '0930' and '1630'\
...
def find_cointegrated_pairs(data):
    n = ps.DataFrame(data).shape[1]
    score_matrix = np.zeros((n, n))
    pvalue_matrix = np.ones((n, n))
    keys = ps.DataFrame(data).keys()
    pairs = []
    i = 'MSFT_adjClose'
    j = 'AAPL_adjClose'
    S1 = ps.DataFrame(data).loc[:,i].dropna().to_numpy()
    S2 = ps.DataFrame(data).loc[:,j].dropna().to_numpy()
            #S1 = data[data.columns[i]].tail(-1)
            #S2 = data[data.columns[j]].tail(-1)
            #S2 = data.columns[j].drop(index=0)
    result = coint(S1, S2)
    score = result[0]
    pvalue = result[1]
    score_matrix[0, 1] = score
    pvalue_matrix[0, 1] = pvalue
    if pvalue < 0.05:
...
data = spark.sql("SELECT MSFT_adjClose, \
                         AAPL_adjClose \
     FROM deltabase.msft_30min_delta \
     FULL JOIN deltabase.aapl_30min_delta ON MSFT_dateTime = AAPL_dateTime \
     WHERE DATE_FORMAT(MSFT_dateTime,'HHmm') = '1630' and DATE_FORMAT(MSFT_dateTime,'yyyy-MM-dd') between '2020-01-03' and '2021-12-31'\
     ORDER BY AAPL_dateTime asc \
     ")
...
#ratios = data['AAPL_adjClose'] / data['MSFT_adjClose']
ratios = S1 / S2
#print(len(ratios))
train = ratios[:10]
test = ratios[10:]
...
ratios_mavg5 = ps.DataFrame(train).rolling(window=5).mean()
ratios_mavg60 = ps.DataFrame(train).rolling(window=60).mean()
std_60 = ps.DataFrame(train).rolling(window=60).std()
...
S1 = data['ADBE_adjClose'].iloc[:1762]
S2 = data['MSFT_adjClose'].iloc[:1762]
...
plt.legend(['ADBE_adjClose','MSFT_adjClose', 'Buy Signal', 'Sell Signal'])
plt.show()
...
trade(data['ADBE_adjClose'].iloc[:1762], data['MSFT_adjClose'].iloc[:1762], 5, 60)

# COMMAND ----------

trade(data['ADBE_adjClose'].iloc[1762:], data['MSFT_adjClose'].iloc[1762:], 5, 90)

# COMMAND ----------

# Find the window length 0-254 
# that gives the highest returns using this strategy
length_scores = [trade(data['ADBE_adjClose_adjClose'].iloc[:1762], 
                data['MSFT_adjClose'].iloc[:1762], 5, l) 
                for l in range(255)]
...
length_scores2 = [trade(data['ADBE_adjClose'].iloc[1762:], 
                  data['MSFT_adjClose'].iloc[1762:],5, l) 
                  for l in range(255)]
